...
def text_to_speech(text):
    js_code = "<script>var msg = new SpeechSynthesisUtterance(); msg.text = '" + text + "'; msg.lang = 'en-US'; window.speechSynthesis.speak(msg);</script>"
    components.html(js_code, height=0)

def set_local_storage(user_id, display_name):
    js_code = "<script>localStorage.setItem('eng_app_userid', '" + user_id + "'); localStorage.setItem('eng_app_name', '" + display_name + "');</script>"
    components.html(js_code, height=0)
...
def load_data():
    try:
        ...
        w_df['id'] = w_df['word'] + "_" + w_df['meaning']
        return w_df, n_df
    except:
        st.error("CSVファイルが読み込めません。")
        st.stop()

WORDS_DF, NETA_DF = load_data()

def get_user_data_by_id(user_id):
    url = FIRESTORE_BASE_URL + "/" + user_id
    try:
...
if "init_done" not in st.session_state:
    ...
    st.session_state.daily_practice_words = unlearned_pool.sample(n=3).to_
