...
if 'page' not in st.session_state:
    st.session_state.update({'page': 'start', 'logged_in': False, 'grade': '中2', 'streak_count': 1, 'session_words': [], 'training_counts': {}, 'current_train_word': None, 'input_key': 0, 'hint_shown': False})
...
def show_menu():
    ...
    # 遷移ボタン：ここで直接関数を呼ぶのではなく、ボタンの状態をチェックする
    if st.button("🚀 練習開始"):
        ...
        if words:
            ...
            st.rerun()
def show_train():
    st.subheader(f"📖 練習: {st.session_state.current_train_word['q'] if st.session_state.current_train_word else '読み込み中...'}")
    ...
    if st.session_state.current_train_word is None:
        ...
        if not pending: st.write("テストへ..."); return
...
if not st.session_state.logged_in:
    ...
    if st.button("スタートする"): st.session_state.logged_in = True; st.rerun()
